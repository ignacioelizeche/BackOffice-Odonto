...
export function PatientDetailContent({ patient: initialPatient }: PatientDetailContentProps) {
  ...
  const handleAddRecord = useCallback(async (toothNumber: number, record: ToothRecord) => {
    try {
      ...
      await patientsService.addDentalRecord(initialPatient.id, toothNumber, {
        ...
        <CardContent className="p-6">
            ...
            <div className="flex items-start gap-4">
              <Avatar className="h-16 w-16">
                <AvatarFallback className="bg-primary text-primary-foreground text-xl font-semibold">
                  {initialPatient.initials}
                </AvatarFallback>
              </Avatar>
              <div>
                <div className="flex items-center gap-3">
                  <h1 className="text-xl font-bold text-card-foreground">{initialPatient.name}</h1>
                  {getStatusBadge(initialPatient.status)}
                </div>
            ...
            <div className="flex flex-col gap-3">
              ...
              <div className="flex flex-wrap gap-3">
                <div className="flex flex-col items-center gap-1 rounded-xl border border-border/50 px-5 py-3">
                  ...
                  <p className="text-lg font-bold text-card-foreground">{initialPatient.totalVisits}</p>
                  <p className="text-xs text-muted-foreground">Visitas</p>
                </div>
                <div className="flex flex-col items-center gap-1 rounded-xl border border-border/50 px-5 py-3">
                  <CreditCard className="h-4 w-4 text-accent" />
                  <p className="text-lg font-bold text-card-foreground">{initialPatient.balance}</p>
              ...
              <Link
                href={`/citas/nueva?patientId=${initialPatient.id}`}
                className="w-full"
              >
          ...
          <div className="mt-5 flex flex-wrap items-center gap-6 rounded-xl border border-border/50 bg-muted/30 px-5 py-3">
            <div className="flex items-center gap-2 text-sm">
              ...
              <span className="font-medium text-card-foreground">{initialPatient.lastVisit}</span>
            </div>
            {initialPatient.nextAppt && (
              <div className="flex items-center gap-2 text-sm">
                <span className="text-muted-foreground">Proxima cita:</span>
                <span className="font-medium text-primary">{initialPatient.nextAppt}</span>
              </div>
            )}
            <div className="flex flex-wrap gap-2">
              {initialPatient.treatments.map((t) => (
